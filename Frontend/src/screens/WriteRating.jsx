  ...
  const handleModalButtonClick = (str) => {
    ...
    for (let i = 0; i < data.task_A.length; i++) {
      if (data.task_A[i][5] === str) {
        ...
        const filteredArrays = data.task_shift_occurs.filter(arr => arr[arr.length - 1] === data.task_A[i][0]);
    ...
    for (let i = 0; i < data.task_B.length; i++) {
      if (data.task_B[i][5] === str) {
        ...
        const filteredArrays = data.task_shift_occurs.filter(arr => arr[arr.length - 1] === data.task_B[i][0]);
        const enabledTask = filteredArrays.some(arr => arr[1] === true);
        if(enabledTask){
          b_tasks[b_tasks.length - 1].push('true');
        }
    ...
    for (var i = 0; i < data.task_C.length; i++) {
      if (data.task_C[i][5] === str) {
        ...
        const filteredArrays = data.task_shift_occurs.filter(arr => arr[arr.length - 1] === data.task_C[i][0]);
        const enabledTask = filteredArrays.some(arr => arr[1] === true);
        if(enabledTask){
          b_tasks[b_tasks.length - 1].push('true');
        }
  ...
  const fetchInfo = useCallback(async () => {
    api
      ...
      .then((response) => {
        ...
        var station = JSON.parse(response.data.station)[0].fields.station_name;
        setStation(station);
      })
      .catch((error) => {
        console.log(error);
